...
void get_display_size(struct fb_info* info)
{
    ...
    if(!w || !h)
    {
        printf("get display size fail: w/h");
        fb_error();
        return;
    }
    info->width = w;
    ...
    if(buf[1]!=MAIL_PROP_RESPONSE_OK)
    {
        printf("get display size fail #2");
        fb_error();
        return;
    }
    info->pitch = buf[5];
...
void get_fb_info(struct fb_info* info)
{
    ...
    while(buf[i]!=0x40001)
    {
        ...
        if(i >= buf_size)
        {
            printf("get fb info fail i > buf_size");
            fb_error();
            return;
        }
    }
    ...
    if(!fb_base || !fb_size)
    {
        printf("get fb info fail base/size");
        fb_error();
        return;
    }
